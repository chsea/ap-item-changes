window.onload = function(){
	$.get('/champions/by-count?sort=post', function(data) {
		...
		info['Number of matches played pre-patch'] = function(data) {
			return data.countPre + ' out of 39873 matches (' + (data.percentPlayedPre.toFixed(4) * 100) + '%)';
		};
		info['Number of matches played post-patch'] = function(data) {
			return data.countPost + ' out of 39952 matches (' + (data.percentPlayedPost.toFixed(4) * 100)  + '%)';
		};
		info.Difference = function(data) {
			return (data.percentPlayedPost - data.percentPlayedPre).toFixed(4) * 100 + '%)';
		};
